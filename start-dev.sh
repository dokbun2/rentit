...
# Gmail 환경 변수 설정
export GMAIL_USER="ggamsire@gmail.com"
export GMAIL_PASSWORD="oerg svup hvto snts"
export ADMIN_EMAIL="dokbun2@gmail.com"

# 실행 중인 5002 포트 프로세스 찾고 종료
echo "실행 중인 5002 포트 프로세스 확인 중..."
PID=$(lsof -t -i:5002)
if [ -n "$PID" ]; then
  echo "포트 5002를 사용 중인 프로세스 (PID: $PID) 종료 중...진행 중..."
  kill $PID
  sleep 1
fi
